...
class Product(Base):
    ...
    feature = Column('feature', String(127), nullable=False)


    def __init__(self, id, category, title, description, price, texture, wash, place, note, story, main_image, feature):
        self.id = id
...
class Variant(Base):
    __tablename__ = 'variant_2.0'
    id = Column('id', Integer, primary_key=True)
    product_id = Column('product_id', BIGINT, ForeignKey('product_2.0.id'), nullable=False)
...
class ProductImages(Base):
    __tablename__ = 'product_images_2.0'
    id = Column('id', Integer, primary_key=True)
    product_id = Column('product_id', BIGINT, ForeignKey('product_2.0.id'), nullable=False)
